...
#define UNTAR_FILE          'x'
// append
#define APPEND_TAR_FILE     'a'
// remove entries
#define REMOVE_TAR_FILE     'r'
// update entries
#define EXTRACT_TAR_FILE    'u'
    ...
    for (unsigned int i = 0; i < filecount; i++) {
        *tar = (tar_t*)malloc(sizeof(struct tar_t));
        ...
        if ((*tar)-> type == DIRECTORY) {
            const size_t len = strlen((*tar)-> original_name); // TODO
...
// 一个用来提供统一的命令接口的函数
static int TarCommand(char command, char* fname, char * path[], int fileCount) {
    int rc = 0;
    const char * filename = fname;
    const char ** files = (const char **)path;
    struct tar_t * archive = NULL;
    int fd = -1;
    char verbosity = 0;     // 不打印详细信息
    ...
    if (command == 'c') {             // create new file
        ...
        switch (command) {
            case 'a':
            rc = tar_write(fd, &archive, fileCount, files, verbosity);
            break;
            case 'd':
            rc = tar_diff(stdout, archive, verbosity);
            break;
            case 'r':
            rc = tar_remove(fd, &archive, fileCount, files, verbosity);
            break;
            case 't':
            rc = tar_ls(stdout, archive, fileCount, files, verbosity + 1);
            break;
            case 'u':
...
int Tar(char * filename, char** path, int fileCount) {
    return TarCommand(TAR_FILE, filename, path, fileCount);
}

int UnTar(char * filename, char** path, int fileCount) {
    return TarCommand(UNTAR_FILE, filename, path, fileCount);
}

int AppenderTar(char * filename, char** path, int fileCount) {
    return TarCommand(APPEND_TAR_FILE, filename, path, fileCount);
}
